...
(async function () {
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID)) {
      ...
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`Your Box Has Not Been Approved Yet. To Submit A Request For Approval :\n—Please Type On Your Thread: ${prefix}request\n—Or Chat the Bot Admin`, event.threadID);
    };
    switch (event.type) {
      case "message":
      case "message_reply":

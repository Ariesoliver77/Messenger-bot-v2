...
module.exports.handleEvent = function({ api, event, client, __GLOBAL, Users }) {
	...
	if (event.body.indexOf("ano?")==0 || (event.body.indexOf("Ano")==0 || (event.body.indexOf("hi bot")==0 || (event.body.indexOf("ano")==0)))) {
			api.sendMessage("Education is the key, kaya magaral ka para d ka tanong ng tanong", event.threadID, (e, info) => {
      setTimeout(() => {
